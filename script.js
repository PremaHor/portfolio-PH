...
(() => {
  ...
  const grid = $("#ph-grid");
  const lb = $("#ph-lightbox");
  const lbImg = $("#ph-lightbox-img");
  const btnClose = $(".ph-lightbox__btn--close", lb);
  const btnPrev = $(".ph-lightbox__btn--prev", lb);
  const btnNext = $(".ph-lightbox__btn--next", lb);

  if (!grid || !lb || !lbImg) return;

  // Přesuň lightbox na konec <body>, aby nebyl v cizím stacking/transform kontextu
  if (lb.parentElement !== d.body) d.body.appendChild(lb);

  const items = Array.from(grid.querySelectorAll(".ph-card"));
  const sources = items.map((a) => ({
    href: a.getAttribute("href"),
    alt: a.querySelector(".ph-card__img")?.alt || "Detail obrázku",
  }));
  ...
  const open = () => {
    lb.classList.add("ph-lightbox--open");
    d.body.classList.add("ph-no-scroll");
  };
  const close = () => {
    lb.classList.remove("ph-lightbox--open");
    d.body.classList.remove("ph-no-scroll");
    lbImg.removeAttribute("tabindex");
  };

  const preload = (i) => {
    const src = sources[(i + sources.length) % sources.length].href;
    const img = new Image();
    img.src = src;
  };
  ...
  async function show(i) {
    ...
    i = (i + sources.length) % sources.length;
    const { href, alt } = sources[i];

    // Preload cílového obrázku
    ...
    if (!lb.classList.contains("ph-lightbox--open")) open();
  ...
  // Delegovaný klik na grid v CAPTURE fázi (před globálními listenery)
  grid.addEventListener(
    "click",
    (e) => {
      const a = e.target.closest(".ph-card");
  ...
  // Klávesnice
  window.addEventListener("keydown", (e) => {
    if (!lb.classList.contains("ph-lightbox--open")) return;
